...
branch: "task/071-refund-policy-faq"
...
- [x] **AC1 — страница.** Given лендинг запущен When открывается `/refund-policy`
...
- [x] **AC2 — ссылки.** Given любая страница When смотрим footer Then в Legal-списке
...
- [x] **AC3 — FAQ.** Given `/` секция FAQ Then присутствуют вопросы: оплата криптой
...
- [x] **AC4 — SEO.** `/refund-policy` есть в `SITE_ROUTES`, в регенерированном
...
- [x] **AC5 — G2.** `npm run lint` + `tsc --noEmit` + `npm run build` + landing e2e
...
- [x] 3 do (TDD: refund-faq.spec.ts RED 6/6 on baseline → GREEN 7/7)
- [x] 4 verify (G2 — lint, tsc/build, unit 16/16, seo:validate 13 routes ok, full e2e 21 passed/2 skipped against prod SSR build; /refund-policy SSR 200 + title curl-verified)
- [x] 5 review (adversarial, pass; non-blocking finding: ToS auto-renewal bullet drift, see Details)
- [x] 5.5 security (skipped — static marketing/legal copy, no auth/input/secrets)
...
(do/verify 2026-06-11, ветка `task/071-refund-policy-faq`. Сверх плана в пределах
touched-файлов: (a) pricing mini-FAQ «Do you offer refunds?» переписан ссылкой на
`/refund-policy` — старый ответ отсылал в ToS и противоречил бы новой политике;
(b) home-FAQ «How does pricing work?» дополнен «Prepay quarterly or annually for a
discount» — консистентность с TASK-047. Найден drift вне scope: ToS Billing bullet
«Auto-renewal» (`terms-of-service.tsx:148-151`) обещает «renew automatically /
cancel from account settings», что расходится с крипто-предоплатой (FAQ-ответ
«How do I cancel» и ADR-004) — не правился (вне declared scope), кандидат в
отдельный однострочный copy-fix. Team-дрейф остаётся вне FAQ: `features.tsx:33`,
`about.tsx:44`, `contact-section.tsx:32`, `privacy-policy.tsx:219`,
`smoke.spec.ts:39,44` — тоже вне scope 071.)
