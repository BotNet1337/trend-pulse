...
- [ ] **AC1 — one-click ссылка.** Given активная pro-подписка истекает через 3 дня
...
- [ ] **AC6 — partially_paid.** Given pending-инвойс на $29 When IPN
...
- [ ] **AC7 — best-effort почта.** Given SMTP/templates падает When partially_paid
...
- [ ] 5.5 security (IPN input + email-ссылки — обязателен)
