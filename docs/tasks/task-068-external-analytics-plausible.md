...
current_step: 7
baseline_commit: "c390c4c"
branch: "task/068-external-analytics-plausible"
lock: ""
- [x] 1 locate (scope + patterns + blast radius)
- [x] 2 plan (G1 — minimal, approved)
- [x] 3 do (TDD: failing test → minimal code)
- [x] 4 verify (G2 — tests + runtime + real behavior)
- [x] 5 review (auto, adversarial — APPROVE: 0 CRITICAL/HIGH, 3 LOW, 2 пофикшены)
- [x] 5.5 security (skipped per test plan — fixed first-party vendor, нет инпута/секретов; security-пункты покрыты review)
- [x] 6 ship (confirm plan done → PR(s))
- [x] 7 learnings (в отчёте оркестратору; docs/learnings.md НЕ трогаем в этой ветке — параллельные PR конфликтуют)
debug_runs: []
...
do/verify 2026-06-11 (Fable):
- Edge case «старый consent»: реализовано ретроактивное выставление
  `plausible_ignore` в mount-эффекте баннера для `consent==='essential'` и для
  `consent==='custom'` с `analytics:false`. Set-only: на маунте ключ никогда не
  снимается (не перетираем явный opt-out), снятие — только явным действием
  пользователя (Accept All / включение toggle + Save).
- **Отклонение от Touch-ONLY (обоснованное):** `landing/src/app/root.tsx` +
  комментарий в `landing/src/shared/cases/cases-context.ts`. Причина: клиент
  гидрировал голый `<RouterProvider>` без `<AppShell>` (с TASK-018), поэтому
  (а) SSR-дерево ≠ клиентское → React #418 на каждой странице, (б) CookieBanner
  НИКОГДА не монтировался на клиенте — баннер был мёртв в проде, AC3 (opt-out
  через toggle) недостижим. Фикс = гидрировать тот же `<AppShell
  cases={window.__INITIAL_STATE__.cases}>`, что рендерит SSR (задокументированный
  контракт TASK-067). После фикса баннер живой, e2e AC3 зелёный.
- Pre-existing (НЕ чинилось, вне scope): остаточный hydration mismatch #418 на
  уровне роутера (`<Suspense fallback={null}>` на клиенте vs `<div>` в SSR-HTML,
  внутри Matches TanStack Router) — воспроизведён на baseline main без правок
  задачи; React восстанавливается клиентским рендером, страница интерактивна.
  В e2e `analytics.spec.ts` (AC2-blocked) этот error отфильтрован с комментарием.
- AC5 локально: реальный `script.js` грузится, `window.plausible` определён,
  pageview и `sign_up_click` доходят до скрипта, но штатно дропаются правилом
  localhost («Ignoring Event: localhost» в консоли) — запрос к `/api/event`
  уйдёт только с боевого домена. Сетевые ассерты в e2e сделаны через стаб
  скрипта + перехват `plausible.io/api/event`.
- Хост-заметка (не код): локальный landing-прод-сервер однажды завис на ответах
  с любым `Accept-Encoding` (включая curl) — лечится рестартом процесса; после
  рестарта compress работает (gzip 200 за ~7ms). К diff отношения не имеет.
